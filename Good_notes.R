...
# addnew column indicating wether automatic
mpg <- 
mpg %>% 
  mutate(auto=grepl("auto",trans))
mod4 <- glm(data=mpg,formula=cty~displ*cyl*auto)
mod5 <- glm(data=mpg,formula=cty~displ*cyl*trans*drv*class*class+year)
...
#predict whether a species in "Gentoo" based on size measurements
...
mod2 <- penguins %>% 
  ...
  glm(data=.,
      ...
      family='binomial')# logistic regression, needs to be T/F and family needs to be binomial,type= response
check_model(mod2)
summary(mod2)
